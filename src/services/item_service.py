...
class ItemService:
    def __init__(self, storage_repository: StorageRepository, validation_service: ValidationService):
    ...
    def set_item_parent(self, board: Board, item_id: ItemId, parent_id: Optional[ParentId]) -> bool:
        if parent_id:
            parent = board.get_parent_by_id(parent_id)
            if not parent:
                raise ValidationError(f"Parent with id '{parent_id}' not found")
    ...
    def get_items_grouped_by_parent(self, board: Board, column_id: ColumnId) -> List[Item]:
        column = board.get_column_by_id(column_id)
        if not column:
            raise ColumnNotFoundError(f"Column with id '{column_id}' not found")
