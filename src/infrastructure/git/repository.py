...
from .branch import BranchType, GitBranch
...
class GitOperations:
        ...
        try:
            ...
            if result.returncode == 0:
                for line in result.stdout.split("\n"):
                    if line.strip():
                        branch = self._parse_branch_line(
                            line, BranchType.LOCAL, current_branch
                        )
                        if branch:
            ...
            if result.returncode == 0:
                for line in result.stdout.split("\n"):
                    if line.strip() and "HEAD ->" not in line:
                        branch = self._parse_branch_line(
                            line, BranchType.REMOTE, current_branch
                        )
                        if branch:
                            branches.append(branch)
        except Exception:
    ...
    def _parse_branch_line(
        self, line: str, branch_type: BranchType, current_branch: Optional[str]
    ) -> Optional[GitBranch]:
