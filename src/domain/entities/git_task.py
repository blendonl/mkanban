...
class GitTask(Item):
    ...
    def should_auto_complete(self) -> bool:
        ...
        return (
            ...
            not self.is_branch_active() and
            self.column_id != "done"  # Avoid moving already completed tasks
        )
    ...
    def should_auto_activate(self) -> bool:
        ...
        return (
            self.auto_sync_enabled and
            self.git_metadata.is_current_branch and
            self.is_branch_active() and
            self.column_id not in ["in-progress", "done"]
        )
